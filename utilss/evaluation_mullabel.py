...
def RankingLoss(label, logit):
    ...
    rankloss = rankloss / N
    return rankloss
def binary_metrics(label_gt_list,label_predict_list):
    f1_score = sklearn.metrics.f1_score(label_gt_list,label_predict_list)
    recall_score = sklearn.metrics.recall_score(label_gt_list, label_predict_list)
    accuracy_score = sklearn.metrics.accuracy_score(label_gt_list, label_predict_list)
    average_precision_score = sklearn.metrics.average_precision_score(label_gt_list, label_predict_list)
    fpr, tpr, thresholds = sklearn.metrics.roc_curve(label_gt_list, label_predict_list)
    auc = sklearn.metrics.auc(fpr, tpr)
    # average_precision_score = sklearn.metrics.average_precision_score(label_gt_list, label_predict_list)
    return f1_score,recall_score,accuracy_score,average_precision_score,auc
